...
//================================================////================================================//
	function CalibrateVolume(InputVolume, BckCorVolumeName) {
		...
		selectWindow(BckCorVolumeName); 
		rename("c"+BckCorVolumeName); // "c" for Calibrate
...
if (BandSuppresionManualRun == false) {
	...
	//run("Gaussian Blur...", "sigma=20");
	run("Median...", "sigma=20");
	imageCalculator("Subtract create 32-bit", "AVG_bck","StDev_bck");
	rename("BckProjection");
}
...
for (n=1; n<=inputSlices; n++) {
	...
	run("Duplicate...", " ");
	run("Size...", "width=inputWidth height=inputHeight depth=1 average interpolation=Bilinear"); 
	rename("SliceBck");

	//getStatistics(null, SliceBckMean, SliceBckMin, SliceBckMax, SliceBckStdev, null);
	getMinAndMax(SliceBckMin, SliceBckMax);
	run("32-bit"); 
	run("Divide...", "value=SliceBckMax");
	setMinAndMax(0, 1);
...
//rename resulted volume
	selectWindow("BckCor-temp"); 
	rename("BC_" + InputVolume);
